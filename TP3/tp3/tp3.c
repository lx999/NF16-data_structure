                        ...
                        element* pred=m.ptr[i];
                         ...
                         if(m.ptr[i]->colonne==j){ //si le premier pointé est celui qu'on cherche
                            ...
                            return;
			                      ...
			                      // dans ce cas nouvelleCase n'est pas celle rajoutée mais remplace
                  			    // la première de la ligne
			                      // m.ptr[i] est celle qui est rajoutée
                            nouvelleCase->valeur=m.ptr[i]->valeur;
                            nouvelleCase->colonne=m.ptr[i]->colonne;
                            nouvelleCase->next=m.ptr[i]->next;
                            m.ptr[i]->valeur=val;
                            m.ptr[i]->colonne=j;
                            m.ptr[i]->next=nouvelleCase;
                            return;
                        }else{
...
int nombreOctetsGagnes(matrice_creuse m1){
   ...
   utilise = m1.Nlignes*sizeof(element*) // les pointeurs en début de ligne
            +n*sizeof(element)              // tous les elements de la matrice
